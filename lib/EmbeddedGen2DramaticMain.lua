...
-- Standalone host ownership.  A user may already have the original Gen-1
-- DRAMATIC_SHAPE enabled. render_pipelines is a record registry, so a second
-- :register("voxel") would collide.  The Gold/Silver standalone package is
-- intentionally authoritative: override an existing record when the API
-- supports it, otherwise register normally on older engines.
local function installPipeline(id, def)
  local registry = mod.content and mod.content.render_pipelines
  if not registry then error("DRAMATIC_SHAPE: render_pipelines registry is unavailable", 0) end
  if type(registry.override) == "function" then
    return registry:override(id, def)
  end
  return registry:register(id, def)
end
...
-- Keep a recoverable renderer error from permanently retiring the engine's
-- `voxel` pipeline. Gen2Recomped itself guards callbacks by marking a pipeline
-- broken for the whole session after one throw. That is appropriate for a
-- normal mod, but this standalone bundle layers Stadium/Wilds integration over
-- a renderer and can safely fall back for one frame while reporting the error.
-- Each callback is therefore fenced here first; the engine sees a clean return
-- instead of a throw and is free to try the next frame/map.
local pipelineErrorOnce = {}
local function protectPipeline(id)
  local registry = mod.content and mod.content.render_pipelines
  local def = registry and registry.get and registry:get(id) or nil
  if type(def) ~= "table" then return end
  local function wrap(name, fallback)
    local raw = def[name]
    if type(raw) ~= "function" then return end
    def[name] = function(...)
      local ok, a, b, c, d = pcall(raw, ...)
      if ok then return a, b, c, d end
      local key = id .. ":" .. name
      if not pipelineErrorOnce[key] then
        pipelineErrorOnce[key] = true
        mod.log:error("embedded Gen-2 %s pipeline %s callback failed: %s", id, name, tostring(a))
      end
      if type(fallback) == "function" then return fallback(...) end
      return fallback
    end
  end
  wrap("available", false)
  wrap("update", nil)
  wrap("drawWorld", nil)
  wrap("worldPresent", function(canvas) return canvas end)
  wrap("present", function(canvas) return canvas end)
  wrap("invalidate", nil)
end
protectPipeline("voxel")
protectPipeline("tiltshift")
...
-- Embedded mode: keep the parent Stadium/Wilds options schema.  The DS
-- setting objects still use their own defaults and the in-game OPTIONS hook
-- below, but replacing the parent schema here would hide STADIUM 2 ROM FILE,
-- 3D VOXEL WORLD, and the roaming-Pokemon controls from the mod manager.
mod.exports.embeddedVoxelOptionSchema = schema

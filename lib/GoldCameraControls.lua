...
--
-- v0.1.80 rotated the user's input by the camera but then quantised it back to
-- Gold's four grid directions.  This module keeps that compatibility fallback
-- for bike/surf/forced/special movement, but NORMAL WALKING in FIRST/THIRD
-- PERSON is continuous: any camera-relative angle, analog magnitude, diagonal
-- keyboard travel, and wall sliding.
--
-- Gold's logical grid still owns gameplay.  Whenever the free body's centre
-- crosses into another cell, the same Gen-2 landing chain is fired: trainer
-- sight, world.stepped, warp, coord script, step counter and wild encounter.
-- Scripts/cutscenes, ledges, connections, forced tiles, bike and surf hand back
-- to the native grid mover instead of being reimplemented here.
local V = ...
...
local function clearFreeState(world, snap)
  ...
  if p then p._stadiumFreeMoving = nil end
...
local function freeTick(world)
  ...
  if mag <= 1e-6 then
    ...
    p._stadiumFreeMoving = nil
    return
  end

  p._stadiumFreeMoving = true
...
function Controls.install()
  ...
  -- Walking cards need their leg frame even though the engine's `moving` flag
  -- is intentionally false during continuous walking (it must stay false so
  -- scripts/events can inspect the logical cell normally).
  if type(Player.walkPhase) == "function" and not Player.stadiumFreeWalkPhaseHook then
    local vanillaWalkPhase = Player.walkPhase
    function Player:walkPhase()
      if self._stadiumFreeMoving then
        local phase = (self.animClock or 0) % (Player.STEP_FRAMES or 16)
        return (phase >= 4 and phase < 12) and 1 or 0
      end
      return vanillaWalkPhase(self)
    end
    Player.stadiumFreeWalkPhaseHook = true
  end
  ...
  World.stadiumCameraRelativeControlsHook = true -- older diagnostics/guards
  Controls.installed = true
  return true
end

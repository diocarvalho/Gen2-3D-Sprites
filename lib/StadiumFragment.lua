...
-- Decode Stadium 2's external skeletal-animation payloads into the same
-- normalized track table returned by StadiumFragment.extract().  `payloads`
-- is the paired animation bank for one Pokemon and `bones` is the skeleton
-- already decoded from that Pokemon's model FRAGMENT.
--
-- GS offsets in these payloads are relative to the payload itself.  Some
-- research/debug dumps expose linked addresses instead, so ptr() also accepts
-- the Stadium FRAGMENT base as a compatibility fallback when it lands inside
-- the current payload.
function StadiumFragment.decodeGSAnimations(payloads, bones)
  if type(payloads) ~= "table" or type(bones) ~= "table" then
    return nil, "invalid Stadium 2 animation inputs"
  end

  local Reader = {}
  Reader.__index = Reader
  function Reader:u8(o)
    return byte(self.d, o + 1)
  end
  function Reader:u16(o)
    local a, b = byte(self.d, o + 1, o + 2)
    if not b then return 0 end
    return a * 256 + b
  end
  function Reader:s16(o)
    local v = self:u16(o)
    if v >= 32768 then return v - 65536 end
    return v
  end
  function Reader:u32(o)
    local a, b, c, d = byte(self.d, o + 1, o + 4)
    if not d then return 0 end
    return ((a * 256 + b) * 256 + c) * 256 + d
  end
  function Reader:ptr(o)
    local p = self:u32(o)
    if p == 0 then return nil end
    if p < #self.d then return p end
    local linked = p - BASE
    if linked >= 0 and linked < #self.d then return linked end
    return p
  end
  ...
  for i = 1, #payloads do
    ...
    local ok, animOrErr = pcall(function()
      if type(payload) ~= "string" or #payload < 0x20 then
        error("payload too short", 0)
      end
      local f = setmetatable({ d = payload }, Reader)
      local header = f:u32(0)
      if header < 0 or header + 0x1C > #payload then
        error("animation header offset is outside payload", 0)
      end
      local a = newGSAnim(f, header)
      if a.nChannels <= 0 or a.nChannels % 3 ~= 0 then
        error("invalid channel count", 0)
      end
      if a.nFrames <= 0 then error("invalid frame count", 0) end
      if not a.chanTable or not a.scaleData or not a.rotData or not a.transData
          or a.chanTable < 0 or a.chanTable >= #payload
          or a.scaleData < 0 or a.scaleData >= #payload
          or a.rotData < 0 or a.rotData >= #payload
          or a.transData < 0 or a.transData >= #payload then
        error("animation data offsets are outside payload", 0)
      end
      ...
      for bi = 1, #bones do
        ...
        if ch >= 0 and a:sampleTrs(ch, 0, b.t, b.r, b.s) ~= nil then
          ...
          for fr = 0, nf - 1 do
            local t, r, sc = a:sampleTrs(ch, fr, b.t, b.r, b.s)
            for k = 1, 3 do
              ts[k][fr + 1], rs[k][fr + 1], ss[k][fr + 1] = t[k], r[k], sc[k]
            end
          end
          tracks[bi] = {
      ...
      return { index = i - 1, frames = nf, flags = a.flags,
               channels = a.nChannels, loopStart = a.loopStart, tracks = tracks }
    end)
    if ok and type(animOrErr) == "table" then
      out[#out + 1] = animOrErr
    else
      errors[#errors + 1] = ("clip %d: %s"):format(i - 1, tostring(animOrErr))
    end
  end

  if #out == 0 then
    return nil, errors[1] or "animation bank contained no decodable clips"
  end
  return out, (#errors > 0) and table.concat(errors, "; ") or nil
end

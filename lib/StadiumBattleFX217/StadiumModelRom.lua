...
-- each 0x10-byte entry indexes that Pokemon's animation list, byte 1 its
-- auxiliary (texture) animation list, and bytes 2/3 are the two effect
-- attachment tags consumed by func_84307F24/func_84307F68.
--
-- Returns every native 0x10-byte row. The first four fields retain their
-- convenient decoded meanings; raw[1..16] is the lossless source row used by
-- the battle synchronizer. aux 0xFF comes back as -1, while raw is untouched.
function Rom:battleRows(species)
  ...
  for e = 0, n - 1 do
    ...
    local raw = {}
    for i = 0, StadiumRom.ENTRY - 1 do
      raw[i + 1] = self:u8(o + e * StadiumRom.ENTRY + i)
    end
    rows[e] = { anim, aux == 0xFF and -1 or aux, raw[3], raw[4], raw = raw }
  end
  rows.n = n
  return rows
end

...
function StadiumBuild.species(rom, fileno)
  ...
  -- Stadium 2 stores the skeletal clips outside the model FRAGMENT in a
  -- parallel animation archive.  A ROM reader that exposes attachAnimations
  -- can replace/augment the model's animation list here; Stadium 1 has no
  -- such method and follows the original path byte-for-byte.
  if type(rom.attachAnimations) == "function" then
    local okAttach, attached, attachErr = pcall(rom.attachAnimations, rom, data, fileno)
    if not okAttach then return nil, tostring(attached) end
